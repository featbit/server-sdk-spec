...
| Maximum events in queue | 10,000 | Capacity of the ingress queue; also the default dispatch buffer capacity. |
...
HTTP send attempts SHOULD use a one-second connection timeout where the runtime supports it and a two-second overall cancellation deadline. These may be fixed or configurable; the overall deadline is not a separate response-read timeout.
...
- Timeouts and timer intervals MUST be positive. Retry delays MAY be zero but MUST NOT be negative. An absent or empty reconnect schedule selects the default.
- A blocking startup configuration MUST satisfy `startup_wait >= connect_timeout`. Equality is permitted.
...
**Hardening requirement:** initialization completion MUST settle exactly once on success, terminal rejection, or explicit close. Caller timeout is separate from that completion. Waiting callers MUST also be awakened on terminal outcomes.
...
The bootstrap format is the same JSON envelope used for synchronization, with data under `data`. The required bootstrap capability applies to offline mode and populates a complete store. A new SDK SHOULD require a `full` bootstrap data set. Supporting bootstrap as an online startup cache is an optional extension and MUST explicitly define when that cache becomes eligible for evaluation.
